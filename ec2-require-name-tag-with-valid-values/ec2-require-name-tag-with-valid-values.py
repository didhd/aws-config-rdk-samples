...
def find_violation(current_tags, reg):
    ...
    for tag in current_tags:
        if tag['key'] == "Name":
            ...
            x = re.search(reg, tag['value'])
            if x:
                value_match = True

            if value_match == False:
                violation = "regex doesn't match"
    if not tag_present:
        violation = "Name tag is not present."
        print(violation)
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    current_tags = configuration_item["configuration"]["tags"]
    violation = find_violation(
        current_tags, valid_rule_parameters["desiredNameTagRegex"])
...
def lambda_handler(event, context):
    ...
    if not compliance_result:
        latest_evaluations.append(
            build_evaluation(event["accountId"], "NOT_APPLICABLE",
                             event, resource_type="AWS::::Account")
        )
