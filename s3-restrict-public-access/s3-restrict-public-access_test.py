...
RULE = __import__('s3-restrict-public-access')
...
class ComplianceTestScenarios(unittest.TestCase):
    ...
    def test_scenario1_invalid_parameters(self):
        ...
        response = RULE.lambda_handler(
            build_lambda_scheduled_event(self.scenario1_invalid_inputs), {})
        assert_customer_error_response(
            self, response, "InvalidParameterValueException")

    def test_scenario2_not_matching_parameters(self):
        RULE.ASSUME_ROLE_MODE = False
        S3_CLIENT_MOCK.PublicAccessBlockConfiguration = MagicMock(
            return_value=self.s3_account_settings)
        response = RULE.lambda_handler(build_lambda_scheduled_event(
            self.scenario2_mismatched_inputs), {})
        resp_expected = []
    ...
    def test_scenario3_not_matching_empty_parameters(self):
        ...
        S3_CLIENT_MOCK.PublicAccessBlockConfiguration = MagicMock(
            return_value=self.s3_account_settings)
        response = RULE.lambda_handler(build_lambda_scheduled_event(
            self.scenario3_empty_inputs_nonmatch), {})
        resp_expected = []
        resp_expected.append(build_expected_response('NON_COMPLIANT', '123456789012', 'AWS::::Account',
                             'BlockPublicAcls:True IgnorePublicAcls:True BlockPublicPolicy:True RestrictPublicBuckets:True'))
        assert_successful_evaluation(self, response, resp_expected)
    ...
    def test_scenario4_empty_inputs_that_match(self):
        ...
        S3_CLIENT_MOCK.PublicAccessBlockConfiguration = MagicMock(
            return_value=self.s3_account_settings)
        response = RULE.lambda_handler(build_lambda_scheduled_event(
            self.scenario4_empty_inputs_match), {})
        resp_expected = []
        resp_expected.append(build_expected_response(
            'COMPLIANT', '123456789012', 'AWS::::Account'))
        assert_successful_evaluation(self, response, resp_expected)
    ...
    def test_scenario5_valid_parameters(self):
        ...
        S3_CLIENT_MOCK.PublicAccessBlockConfiguration = MagicMock(
            return_value=self.s3_account_settings)
        response = RULE.lambda_handler(
            build_lambda_scheduled_event(self.scenario5_valid_inputs), {})
        resp_expected = []
        resp_expected.append(build_expected_response(
            'COMPLIANT', '123456789012', 'AWS::::Account'))
        assert_successful_evaluation(self, response, resp_expected)
