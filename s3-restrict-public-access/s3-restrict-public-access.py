...
Rule Parameters:
  ...
  | ---------------------- | ---------- | -----------------------------------------------------------------------------------------|
  | BlockPublicAcls        | Optional   | Block new public ACLs and uploading public objects (Default True)                        |
  | IgnorePublicAcls       | Optional   | Remove public access granted through public ACLs (Default True)                          |
  | BlockPublicPolicy      | Optional   | Block new public bucket policies (Default True)                                          |
  | RestrictPublicBuckets  | Optional   | Block public and cross-account access to buckets that have public policies (Default True)|
  | ---------------------- | ---------- | -----------------------------------------------------------------------------------------|
Feature:
...
Scenarios:
  Scenario 1:
    Given: Given: Input paramters are configured
      And: At least one Input Paramter is invalid
  ...
  Scenario 3:
    Given: Given: No input parameters are defined
      And: At least 1 default value does not match the corresponding value for the account
     then: Return NON_COMPLIANT
  Scenario 4:
    Given: Given: No input parameters are defined
      And: All S3 public access account settings match the default values
...
# hack to change the version of boto
sys.path.insert(0, ENVLAMBDATASKROOT+"/boto3-1-9-82")
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if (response['PublicAccessBlockConfiguration']['BlockPublicAcls'] == valid_rule_parameters['BlockPublicAcls']) \
        ...
        evaluations.append(build_evaluation(
            event['accountId'], 'COMPLIANT', event))
        ...
        annotationbuilder = 'BlockPublicAcls:' + str(response['PublicAccessBlockConfiguration']['BlockPublicAcls']) + ' ' \
            ...
            str(response['PublicAccessBlockConfiguration']
                ['RestrictPublicBuckets'])
        evaluations.append(build_evaluation(
            event['accountId'], 'NON_COMPLIANT', event, annotation=annotationbuilder))
    return evaluations
...
def evaluate_parameters(rule_parameters):
    ...
    valid_rule_parameters['BlockPublicAcls'] = to_bool(
        rule_parameters['BlockPublicAcls'])
    valid_rule_parameters['IgnorePublicAcls'] = to_bool(
        rule_parameters['IgnorePublicAcls'])
    valid_rule_parameters['BlockPublicPolicy'] = to_bool(
        rule_parameters['BlockPublicPolicy'])
    valid_rule_parameters['RestrictPublicBuckets'] = to_bool(
        rule_parameters['RestrictPublicBuckets'])
